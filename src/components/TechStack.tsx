...
const TechStack = () => {
  return (
    <section className="flex flex-col items-center justify-center px-6 py-24 relative">
      <h2 className="text-4xl md:text-6xl font-bold text-center mb-4 text-glow">Tech Stack</h2>
        ...
        <div className="flex flex-wrap justify-center gap-8 items-center">
          {technologies.map((tech, index) => (
            ...
            >
              {/* Tooltip */}
              <span
                className="absolute -top-9 left-1/2 -translate-x-1/2 px-2 py-1 rounded-lg text-slate-50 dark:text-white text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10"
                style={{ background: "var(--tooltip-bg)", backdropFilter: "blur(8px)" }}
              >
                {tech.name}
              </span>
              <img
