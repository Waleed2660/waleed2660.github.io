  ...
  const projects: Project[] = [
    {
      ...
      description:
        "An open-source enterprise-grade Spring Boot application designed to help developers bridge the gap between tutorials and real-world development.",
      tech: [
        "Spring Boot",
        "ActiveMQ",
        "PostgreSQL",
    ...
    },
    {
      ...
      description:
        "A k6 extension for publishing JSON messages to Confluent Kafka via the Confluent REST Proxy with full OAuth 2.0 (Client Credentials) support. Created as an alternative to xk6-kafka, whose underlying go-kafka library lacks OAuth 2.0 support.",
    ...
    },
    {
      title: "Nimbus",
      description:
        "Personal Cloud Storage webapp built with JS, React, Tailwind CSS. Allows file upload, download & directory navigation. Backend application is built with Java SpringBoot which uses Amazon S3 to store files.",
    ...
    },
    {
      ...
      description:
        "Created a script using Tello SDK & API to control DJI Tello drone with an Xbox Controller, including live video feed streaming from the drone's camera to your device.",
    ...
    },
    {
      title: "Handy",
      description:
        "Inspired by Apple Vision Pro, this project uses Google's Mediapipe framework and OpenCV to detect hand gestures to control your desktop cursor, supporting actions like pinching and dragging.",
    ...
    },
    {
      title: "C.O.G.S",
      description:
        "A 2D platform fighter game inspired by classic titles like Mario & Contra, featuring action-packed gameplay and retro-style graphics. Built using Java and JSFML (Java Simple and Fast Multimedia Library).",
                  ...
                  <div className="flex flex-col md:flex-row gap-8">
                    ...
                    <div className="flex flex-col flex-1">
                      <div className="flex items-center gap-3 mb-1 flex-wrap">
                        ...
                        {stats && (
                          <span className="glass rounded-xl px-3 py-1 text-sm text-yellow-700 dark:text-yellow-300/90 whitespace-nowrap">
                            ★ {stats.stars}
                          </span>
                        )}
                        {stats && stats.forks > 0 && (
                          <span className="glass rounded-xl px-3 py-1 text-sm text-slate-500 dark:text-white/50 whitespace-nowrap">
                            ⑂ {stats.forks}
                          </span>
                        )}
                      </div>
                      <p className="text-slate-600 dark:text-white/70 mb-4 leading-relaxed">
                        {project.description}
                      </p>
                      {project.highlights && (
                      ...
                      <div className="flex justify-between items-end mt-auto">
                        <div className="flex flex-wrap gap-2">
                          {project.tech.map((tech, ti) => (
                            <div
                              key={ti}
                              className="glass rounded-xl px-3 py-1 text-slate-700 dark:text-white/80 text-sm inline-block"
                            >
                              {tech}
                            </div>
                          ))}
